...
console.log("tornadoInstance is ",tornadoInstance)  
...
async function main() {
  if (inBrowser) {
    ...
    program
      ...
      .action(async (str) => {
        await init({ rpc: program.rpc, currency: "eth", amount:"1" })
        await runTask({str})
      })
...
async function runTask({ str }) {
  let rpcEvents = await tornadoContract.getPastEvents('Deposit', {
    fromBlock: str,
    toBlock: 'latest'
  })	
  console.log(`rpcEvents length `, rpcEvents.length)
   rpcEvents = rpcEvents.map(({ blockNumber, transactionHash, returnValues }) => {
    const { commitment, leafIndex, timestamp } = returnValues
  ...
  const leaves = rpcEvents
    ...
    .map((e) => {
      const index = toBN(e.leafIndex).toNumber()
	  console.log("leaf index is ", index)
    })
}
