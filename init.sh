...
async function main() {
  const release = await axios.get('https://api.github.com/repos/tornadocash/tornado-core/releases/22284165')
...
async function loadWithdrawalData({ amount, currency, deposit }) {
  try {
	...
	// should is isLocalRPC to check for the decimal place, not config.js
